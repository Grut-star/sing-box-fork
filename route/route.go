...
func (r *Router) RouteConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext) error {
    ...
    if sharedfirewall.GlobalInterceptor != nil && metadata.ProcessInfo != nil {
    	// Прямое подключение означает, что Host пустой (не было DNS-резолва),
    	// но при этом Destination — это валидный IP-адрес.
        if metadata.Domain == "" && metadata.Destination.IsValid() && metadata.Destination.IsIP() {

    		ipStr := metadata.Destination.AddrString()
    		...
    		if !isIpAllowed {
    			...
    			// Закрываем соединение, имитируя RST
    			conn.Close()
    			return errors.New("Direct IP blocked by NekoDesk Firewall")
    		}
        }
    }
...
func (r *Router) RoutePacketConnection(ctx context.Context, conn N.PacketConn, metadata adapter.InboundContext) error {
    ...
    if sharedfirewall.GlobalInterceptor != nil && metadata.ProcessInfo != nil {
    	if metadata.Domain == "" && metadata.Destination.IsValid() && metadata.Destination.IsIP() {
    		...
    		if !isIpAllowed {
    			sharedfirewall.GlobalInterceptor.NotifyDirectIpBlocked(uid, ipStr)
    			// В UDP мы просто не пересылаем пакет дальше
    			return errors.New("Direct IP blocked by NekoDesk Firewall")
    		}
        }
    }
