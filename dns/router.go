...
import (
    ...
    "github.com/Grut-star/shared-firewall"
...
func (r *Router) Exchange(ctx context.Context, message *mDNS.Msg, options adapter.DNSQueryOptions) (*mDNS.Msg, error) {
    ...
    // Проверяем: есть ли перехватчик, известен ли UID и есть ли вопрос в DNS пакете
    if sharedfirewall.GlobalInterceptor != nil && metadata != nil && len(message.Question) > 0 {
        ...
        fmt.Printf("[FIREWALL-GO] Перехват DNS -> %s (UID: %d)\n", domain, uid)

    	// Отправляем запрос в Android (поток здесь остановится и будет ждать)
    	allowed := sharedfirewall.GlobalInterceptor.AskDnsPermission(uid, domain)
    	if !allowed {
    	    fmt.Printf("[FIREWALL-GO] Блокировка DNS -> %s\n", domain)
    		// Если пользователь запретил, возвращаем пустой ответ или ошибку
    		// todo Просмотреть как именно видно блокировку dns и поправить если нужно
    		return nil, errors.New("DNS blocked by NekoDesk Firewall")
    	}
    }
